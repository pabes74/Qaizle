...
- `.github/scripts/pr-quiz.mjs` — runs on `pull_request` (opened/synchronize/reopened/ready_for_review). Fetches the PR + changed files, builds a prompt (title, description, per-file patch excerpts truncated to 1500 chars), and generates questions through one of two provider functions selected by `QUIZ_PROVIDER` (`requestQuestionsOnce` dispatches to `requestQuestionsFromCopilot` — GitHub Models inference endpoint, prompt-instructed JSON — or `requestQuestionsFromClaude` — Anthropic Messages API using `output_config.format` structured outputs for guaranteed-valid JSON), then normalizes the response into exactly 5 questions with 4+ options each via the shared `normalizeQuestions()`. **The two providers fail differently by design:** if `provider` is `copilot` and generation fails twice, it falls back to a hardcoded question set (`fallbackQuestions()`) and posts it with a visible ⚠️ warning banner; if `provider` is `claude`, a failure instead creates a `failure` check run (`createFailedCheckRun`) and exits non-zero — no quiz comment is posted at all, since a broken *explicit* Claude configuration shouldn't look like a working quiz. Shuffles each question's options (Fisher-Yates) so the correct answer isn't always first. Renders a markdown comment and **upserts** it (finds an existing comment by the `<!-- copilot-pr-quiz -->` HTML marker and PATCHes it, otherwise creates one) so re-runs on `synchronize` update the same comment instead of duplicating. Also opens an `in_progress` "Copilot PR Quiz" check run on the PR head SHA.
